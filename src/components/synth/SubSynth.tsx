...
export default function SubSynth() {
  ...
  const handleAutoSubGen = () => {
    ...
    const updatedSteps = Array(32).fill(null).map((_, i) => {
      // Classic EDM sidechained offbeat pattern (mainly step 2, 6, 10, 14, 18, 22, 26, 30 of 32 steps, which is offbeat of bass drum)
  ...
  return (
    <div className="flex flex-col gap-6" id="sub-synth-panel">
      ...
      {/* 2. Synthesizer Hardware Knobs Rack - arranged horizontally above the grid */}
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 bg-slate-900/40 p-4 rounded-xl border border-slate-850" id="sub-synth-tweaker-rack">
        {/* A. Waveform Selector */}
        <div className="bg-slate-950 p-3 rounded-lg border border-slate-850 flex flex-col justify-between">
          <span className="text-[9px] font-mono text-slate-400 font-bold uppercase mb-1.5 block">OSC WAVE:</span>
        ...
        {/* B. Glide & Saturation Controls */}
        <div className="bg-slate-950 p-3 rounded-lg border border-slate-850 flex items-center justify-around gap-2">
        ...
        {/* C. Analog Filter Envelope */}
        <div className="bg-slate-950 p-3 rounded-lg border border-slate-850 flex items-center justify-around gap-2">
          <div className="flex flex-col items-center">
            <span className="text-[8px] font-mono text-slate-400 uppercase mb-2">CUTOFF</span>
            <Knob
              min={40}
              max={450}
          ...
          <div className="flex flex-col items-center">
            <span className="text-[8px] font-mono text-slate-400 uppercase mb-2">RESONANCE</span>
            <Knob
              min={0.5}
              max={12}
        ...
        {/* D. ADSR Sliders - spanning 2 columns in md layout */}
        <div className="col-span-1 md:col-span-2 bg-slate-950 p-3 rounded-lg border border-slate-850 flex flex-col justify-between">
          <span className="text-[8px] font-mono text-indigo-400 font-black uppercase mb-1.5 block font-bold">VOLUME ADSR ENV:</span>
          <div className="grid grid-cols-4 gap-2 text-center h-20">
            {([
              ...
              return (
                <div key={key} className="flex flex-col items-center justify-between h-full">
                  <input
                    ...
                    className="accent-indigo-400 h-10 w-1 bg-slate-800 rounded appearance-none"
                    style={{ writingMode: 'vertical-lr', direction: 'rtl' }}
                    title={`${label.toUpperCase()}: ${currentVal}`}
                  />
                  <span className="text-[8px] font-mono text-slate-400 font-bold mt-1">{label}</span>
                </div>
              );
            })}
          </div>
      ...
      {/* 3. Step Sequencer Piano Roll Grid (Full Width & dynamically sizing) */}
      <div className="bg-slate-900/40 p-4 rounded-xl border border-slate-850 overflow-x-auto min-w-0" id="sub-synth-sequence-grid">
        <div className="flex flex-col gap-1.5" style={{ minWidth: (state.project.patternLength || 16) * 36 + 80 }}>
          {/* Playhead indicators top bar */}
          <div className="flex items-center gap-1 pl-16">
            {Array(state.project.patternLength || 16).fill(null).map((_, i) => (
              <div 
                key={i} 
                className={`flex-1 h-1.5 rounded-sm transition-all duration-75 flex flex-col justify-between items-center ${
                  state.currentStep % (state.project.patternLength || 16) === i && state.isPlaying 
          ...
          {SUB_KEYS.map((key) => (
            <div key={key} className="flex items-center gap-1.5" id={`sub-pitch-row-${key}`}>
              ...
              <div className="flex-1 flex gap-1 h-8">
                {Array(state.project.patternLength || 16).fill(null).map((_, stepIdx) => {
                  const stepItem = sub.steps[stepIdx] || { active: false, note: 'C', octave: 1 };
                  const isNoteActive = stepItem.active && stepItem.note === key;
                  const isPlayhead = state.currentStep % (state.project.patternLength || 16) === stepIdx && state.isPlaying;
